...
// Страница одногруппника: провал из ленты по аватару/имени. Только чтение.
...
export function AthleteProfilePage() {
  ...
  const { data, loading, error } = useApi<AthleteProfileData>(`/api/athlete/athletes/${params.id}`);

  const [extraWorkouts, setExtraWorkouts] = useState<AthleteProfileData["workouts"]>([]);
  const [nextCursor, setNextCursor] = useState<AthleteProfileData["nextCursor"]>(null);
  ...
  async function loadMore() {
    ...
    try {
      ...
      const moreData = await api<AthleteProfileData>(
        `/api/athlete/athletes/${params.id}?${search.toString()}`
      );
  ...
  if (error || !data) {
    return (
      <div className="card">
        ...
        <Link to="/athlete" className="button primary-button">Назад</Link>
      </div>
    );
  }
  ...
  return (
    <div className="stack">
      <section className="athlete-account-header-shell">
        <Link to="/athlete" className="inline-link">Назад</Link>
      ...
      <AthleteTrends trendsEndpoint={`/api/athlete/athletes/${params.id}/trends`} />
      ...
      <section className="card trainer-dashboard-list-section">
        ...
        {allWorkouts.length === 0 ? (
          <div className="trainer-dashboard-leader-empty">
            ...
            <div className="muted">Пробежки появятся после синхронизации с часами спортсмена.</div>
          </div>
        ) : (
